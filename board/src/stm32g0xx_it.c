...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef* spi) {
    if (SPI1 == spi->Instance) {
        // Unlock LEDS
        task_immediate(leds_tx_complete_handler);
    }
...
void HAL_I2C_MasterTxCpltCallback(I2C_HandleTypeDef* hi2c) {
    ...
    task_immediate_signal(i2c_complete_handler, status);
}
