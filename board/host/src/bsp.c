...
static void MX_SPI2_Init(void) {
    ...
    HAL_GPIO_Init((GPIO_TypeDef*)FLASH_CS.port, &GPIO_InitStruct);
...
static void MX_GPIO_Init(void) {
    ...
    HAL_GPIO_WritePin((GPIO_TypeDef*)AUDIO_SD.port, AUDIO_SD.pin,
                      GPIO_PIN_RESET);

    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin((GPIO_TypeDef*)MUXRST.port, MUXRST.pin, GPIO_PIN_RESET);
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init((GPIO_TypeDef*)ANALOG_RNG.port, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BAT_READ.pin;
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init((GPIO_TypeDef*)BAT_READ.port, &GPIO_InitStruct);
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init((GPIO_TypeDef*)TURN_SW.port, &GPIO_InitStruct);
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init((GPIO_TypeDef*)HIDE_TGL.port, &GPIO_InitStruct);
    ...
    HAL_GPIO_Init((GPIO_TypeDef*)AUDIO_SD.port, &GPIO_InitStruct);
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init((GPIO_TypeDef*)MUXRST.port, &GPIO_InitStruct);
}
...
void serial_write(const buffer_t data, length_t length, callback_t oncomplete) {
    ...
    if (length <= MAX_SERIAL_PACKET) {
        ...
        if (HAL_OK != HAL_UART_Transmit_IT(&huart1, serial_buffer, length)) {
            serial_lock = false;
        }
    }
}
...
void serial_printhex(const buffer_t data, length_t length,
    ...
    if ((length * 4) <= MAX_SERIAL_PACKET) {
        uint8_t* write_ptr = serial_buffer;
        ...
        serial_txcomplete = oncomplete;
        if (HAL_OK !=
            HAL_UART_Transmit_IT(&huart1, serial_buffer, length * 4)) {
            serial_lock = false;
        }
    }
}
