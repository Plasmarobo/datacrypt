...
class TestGroup {
   public:
    explicit TestGroup(IEventListenerPtr&& reporter, Config const* config)
        ...
        m_tests = createShard(m_tests, m_config->shardCount(),
                              m_config->shardIndex());
    }
...
Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(
    ...
    return Detail::make_unique<TestCaseInfo>(_className, nameAndTags,
                                             _lineInfo);
}
...
void ReporterRegistry::registerReporter(std::string const& name,
    ...
    CATCH_ENFORCE(ret.second, "reporter using '" + name +
...
struct GeneratorTracker final : TestCaseTracking::TrackerBase,
    ...
    void close() override {
        ...
        if (should_wait_for_child || (m_runState == CompletedSuccessfully &&
...
Totals RunContext::runTest(TestCaseHandle const& testCase) {
    ...
    do {
        ...
        m_testCaseTracker = &SectionTracker::acquire(
            m_trackerContext, TestCaseTracking::NameAndLocationRef(
...
void ReporterBase::listTests(std::vector<TestCaseHandle> const& tests) {
    defaultListTests(m_stream, m_colour.get(), tests,
                     m_config->hasTestFilters(), m_config->verbosity());
}
