...
static void MX_GPIO_Init(void) {
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init(TURN_SW_GPIO_Port, &GPIO_InitStruct);
...
void display_set_img(uint8_t x, uint8_t y, buffer_t img, uint8_t width,
                     uint8_t height, callback_t on_complete) {
    // Write I2C1
}
...
bool gpio_set(gpio_t gpio, bool set) {
